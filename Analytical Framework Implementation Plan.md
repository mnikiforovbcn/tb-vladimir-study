...
### Phase 5 — Visualization (`viz.py`) (complete)
1. Confirm `plotly`, `kaleido`, `matplotlib`, and `seaborn` are installed in the `uv` environment — all four are declared in `pyproject.toml`, but its `kaleido>=0.2.1` floor is far below the `1.3.0` actually resolved in `.venv`, and kaleido's 1.0 release changed its internal architecture in ways that can break a naive `fig.write_image()` call. Smoke-test a bare `import` of each, plus one real `fig.write_image(tmp_path)` PNG export, before writing chart code against them. Also confirm whether `matplotlib`/`seaborn` are actually needed by this module (every item below is Plotly-only) or are vestigial/meant for a different phase; drop the dependency or document its real purpose accordingly. — Done: all four import cleanly under the resolved `kaleido` 1.3.0; a real `fig.write_image(tmp_path)` PNG export round-tripped with no special-case code needed. `matplotlib`/`seaborn` turned out unused by every item below (the module is Plotly-only end to end); the dependency was left in place rather than removed, since dropping it is a `pyproject.toml` change outside this module's scope.
2. Shared cross-cutting helpers, written first so items 3–8 don't each invent their own convention: `_SITE_COLORS` (a fixed `{"Vladimir": ..., "Kovrov": ..., "Murom": ...}` mapping, reused anywhere `Source` is a dimension) and a suppressed-cell policy — any row with `suppressed=True` (per `cascade.suppress_small_cells`) is excluded from the plotted geometry (its value columns are already `pd.NA`), and every chart gets a one-line caption stating how many cells were suppressed, mirroring the privacy note `run_cascade.py` already prepends to `cascade_report.md`. — Done: `_SITE_COLORS`, `_drop_suppressed`, `_suppression_caption`, `_add_suppression_caption`, and a shared `_CAPTION_ANNOTATION_STYLE` implemented once at the top of `viz.py`; every chart/table function below builds on these rather than reinventing them.
3. `funnel_chart(cascade_df)` — `plotly.graph_objects.Funnel` over `cascade.step2_screening_cascade(df)`'s long-format output (`stage`, `n`, `count`, `pct`, `ci_low`, `ci_high`, `suppressed`), one bar per stage in `cascade._STEP2_STAGES`' order (screened → suspected_tb → diaskintest_positive → full_eval), plotted by `pct` (not raw `count`) so the funnel is comparable across groups of different size. — Done: implemented with a `_validate_step2_stages` guard (raises on the wrong stage set/shape) and a shared `_step2_funnel_trace` helper reused by item 4.
4. `funnel_chart` per-`TargetGroup` small multiples — same input shape as item 3 but called with `group_by=["TargetGroup"]`, rendered as one funnel per facet in a subplot grid rather than a single overlaid figure; a distinct layout problem from item 3, not a parameter tweak on the same figure. — Done: `funnel_chart_by_group(cascade_df, group_col="TargetGroup")` via `plotly.subplots.make_subplots`, facets ordered by `cascade._LABEL_MAPS[group_col]`'s own value order when available (falls back to alphabetical for an uncoded `group_col` such as `Source`).
5. `outcome_stacked_bar(df, group_by=["Source"])` — outcome composition bars over `cascade.step6_adherence_completion(df, group_by=...)["outcome_distribution"]`'s `category`/`count`/`pct` columns (the `outcome_branch` labels). `group_by` accepts any `cascade._ALLOWED_GROUP_DIMS` combination (e.g. `["Source"]`, `["TargetGroup"]`, or both), not just `Source` — this also satisfies Descriptive Study Plan §9's "by site and target group" and its "small multiples comparing the three sites" bullets, so no separate small-multiples function is needed for Step 6. — Done: implemented with fixed `_OUTCOME_BRANCH_LABELS`/`_OUTCOME_BRANCH_COLORS` stack/legend/color order; 0, 1, 2, and 3+ `group_by` dims all handled (3+ falls back to one concatenated `"v1 | v2 | ..."` x-label, since Plotly's grouped-category axis only supports two levels).
6. `trend_lines(trend_df)` — line chart over `trends.step9_quarterly_trends(df)`'s long-format output (`metric`, `year`, `quarter`, `n`, `suppressed`), one line per `metric` (`enrollment`/`treatment_initiation`/`outcome`). Per `trends.py`'s own docstring, quarters with zero events never appear as rows — this function must reindex against the full observed quarter range and fill missing quarters with `n=0` before plotting, or the line will silently skip gaps instead of showing them as zero. — Done: `_full_quarter_range` reindexes every metric against the shared observed range; a true zero-event quarter is filled with `n=0`, while a *suppressed* quarter (row existed, value already blanked) stays dropped via `_drop_suppressed` rather than becoming a fabricated zero — confirmed by a hand-verified test against the real fixture's quarterly counts.
7. `baseline_table(step1_dict)` — presentation table for Step 1 (`cascade.step1_baseline_table`'s `table1`/`age_summary`/`contacts_per_index_case`), satisfying Descriptive Study Plan §9's "Table 1-style summary table of baseline characteristics by site" bullet, which the original Phase 5 draft did not cover. Must carry forward the same explicit suppression caveat `run_cascade.py` already attaches to `table1` (not yet suppression-safe — tracked as item 7's follow-up) rather than silently presenting it as safe. — Done: `_table1_to_table` flattens `TableOne`'s two-level row/column `MultiIndex` into a `go.Table` and attaches the carried-forward `_TABLE1_SUPPRESSION_CAVEAT`; `_summary_row_to_table` renders `age_summary`/`contacts_per_index_case` with the module's normal suppression caption, since those two are already suppression-safe.
8. `site_comparison_table(step10_dict)` — Step 10 side-by-side site summary, rendered as a styled DataFrame (`pandas.Styler`) or Plotly table per sub-table. Takes `cascade.step10_site_comparison(df, analysis_date)`'s actual return shape directly: a nested dict of roughly a dozen DataFrames (`screening_cascade`, `diagnostic_outcomes`, `lti_cascade` ×3, `regimen`, `adherence_completion` ×3, `incentive_uptake` ×2, `followup_outcomes` ×6) — renders each sub-table as its own labeled block (mirroring `run_cascade.py`'s `_section` pattern), not a single flat table. — Done: `_STEP10_TITLES` (titles copied verbatim from `run_cascade.py`'s own Step 10 section) plus a generic, column-agnostic `_dataframe_to_table` renderer; `site_comparison_table` mirrors `step10_dict`'s own shape (plain figure where the input had a plain DataFrame, nested dict where it had one). `followup_outcomes` is ×7 sub-tables in the real implementation, not ×6 as originally estimated here (`step8_followup_outcomes` returns 7 keys, including a separately-listed `incidence_rate`).
9. Every figure function (items 3–6) returns a figure object, never a saved file. A separate `export(fig, path)` helper saves both a static PNG (`kaleido`) and interactive HTML from that one figure — static for the embedded report/Word export (Phase 6), interactive for stakeholder review. — Done: `export(fig, path)` treats `path` as a base path (any suffix replaced), creates the parent directory if missing, and writes `<base>.png`/`<base>.html` via plain `fig.write_image()`/`fig.write_html()` — verified against the real installed kaleido 1.3.0 for both `go.Funnel`/`go.Bar` figures and `go.Table` figures.
10. Unit tests: each chart/table function called on the synthetic fixture and on a deliberately all-suppressed group, asserting it doesn't raise and that suppressed cells are excluded from the plotted trace data (not just visually hidden); `export()` tested by asserting a real PNG and HTML file land on disk with nonzero size. — Done: `tests/test_viz.py` (18 tests), happy paths hand-verified against the real fixture numbers already confirmed in `tests/test_cascade.py`/`tests/test_trends.py`, plus deliberately-all-suppressed pairs per function and a few contract/raises tests; 18/18 passed in the project's real `uv` venv (`uv run python -m pytest tests/test_viz.py -v`).

### Phase 6 — Report assembly (`report/descriptive_report.qmd`, `report.py`) (complete)
1. Author one Quarto document parameterized by `run_date` and `analysis_ready_path`, organized to mirror Descriptive Study Plan §7 (Steps 1–10) and §9 (visualization plan) section-for-section, so a reviewer can check the report against the study plan line by line. — Done: `report/descriptive_report.qmd` built on the `tb-cascade` Jupyter kernel, with a Quarto/Papermill `parameters` cell (`as_of`, `analysis_ready_path`, `window_months`) and one section per Step 1–10 in Descriptive Study Plan §7's own order; confirmed rendering end-to-end with no errors on the real dataset.
2. Each section calls the Phase 4/5 functions directly (no copy-pasted numbers) — the report is generated code, not hand-edited prose. — Done: every section calls the real `cascade.py`/`trends.py`/`viz.py` functions; sections with a dedicated Phase 5 renderer (Steps 1, 2, 6, 9, 10) use it, sections without one (Steps 3, 4, 5, 7, 8) render via the report's own `show_table()` helper (a thin `IPython.display(Markdown(df.to_markdown()))` wrapper mirroring `run_cascade.py`'s `_df_md`/`_section` convention).
3. Render targets: HTML (primary, interactive Plotly embeds) and a static Markdown/PDF version for circulation to reviewers who need a flat file. If a Word deliverable is later required, render to Markdown first and run it through the existing `docx` skill/pipeline rather than maintaining a second template. — Done: `format: {html, gfm}` both configured (`embed-resources: true`, `code-fold`/`code-tools` for html); `quarto render report/descriptive_report.qmd` verified twice against the real dataset, producing both `descriptive_report.html` and `descriptive_report.md` with zero errors. PDF target not built — not requested beyond the original Markdown/PDF "either" framing, and gfm already serves the flat-file circulation need.
4. Auto-prepend a header block stating the data snapshot date, pipeline git commit hash, and row counts in/out of each QC rule, so every report is self-describing. — Done: a `report-header` cell prints `as_of`, the data snapshot's own file-mtime date, the pipeline's short git commit hash (`+dirty` suffix if the working tree has uncommitted changes, `"unknown"` outside a git checkout), and record count; a follow-up `report-header-qc` cell shows `qc.run_qc(raw).summary`'s per-rule `n_checked`/`n_violations` table (never the record-level `flagged` table, which carries `Source`/`Nomer` linkage keys). Verified by a real render.

### Phase 7 — Automation and CLI (complete)
1. `cli.py` exposes one command, e.g. `python -m tb_cascade.cli run --as-of 2026-06-16`, chaining Phases 1→6 and writing everything to `reports/<run_date>/`. — Done: `src/tb_cascade/cli.py` implements `python -m tb_cascade.cli run`, chaining ingestion (Phase 1) → schema/QC (Phase 2) → derived variables (Phase 3) → Quarto report rendering (Phases 4–6, exercised inside the `.qmd` templates) into `reports/<run_date>/`; also adds `--run-date`, `--window-months`, `--lang` (repeatable), and `--skip-report` beyond the minimal spec. Verified both via a sandboxed reimplementation of `tests/test_cli.py`'s 6 scenarios and for real on the user's machine against the full 7,732-row dataset — ingestion, schema validation, QC (2,331 flagged rule/record pairs), and the 96-column derived table all completed correctly. The Quarto render step initially failed there with a missing `papermill` dependency (Quarto's `-P` parameter injection requires it); fixed by adding `papermill>=2.6` to `pyproject.toml`'s `dev` extras.
